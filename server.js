...
if (activeProvider() === 'console') {
  ...
  if (process.env.NODE_ENV === 'production' && process.env.REQUIRE_MAIL_PROVIDER !== '0' && process.env.ALLOW_NO_MAIL_PROVIDER !== '1') {
    console.error('\x1b[31m⛔ التشغيل اتوقف: مفيش مزوّد بريد متظبط في الإنتاج (RESEND_API_KEY / SMTP_URL / MAIL_WEBHOOK_URL). لو ده مقصود شغّل ALLOW_NO_MAIL_PROVIDER=1.\x1b[0m');
    process.exit(1);
  }
}
...
if (!instanceLock.ok && !MULTI_INSTANCE) process.exit(1);
// (إصلاح) تشغيل أكتر من نسخة بقى مدعوم بشكل صحيح على نفس الـ Volume:
//  - SQLite شغالة WAL + busy_timeout، والكتابات كلها transactions ذرّية.
//  - النسخة اللي معاها القفل بس هي «القائد» (leader): هي اللي بتشغّل
//    المهام الدورية (إشعارات مجدولة، تنبيه الطلبات المعلّقة، النسخ
//    الاحتياطي). الباقي بيخدم الطلبات بس، فمفيش إشعار مكرر ولا نسختين
//    احتياطيتين في نفس اللحظة.
//  - كل حدود المعدّل بتتخزّن في القاعدة المشتركة في وضع تعدّد النسخ.
const IS_LEADER = instanceLock.ok;
if (!IS_LEADER) console.warn('\x1b[33mℹ️  النسخة دي بتشتغل كـ follower: بتخدم الطلبات بس من غير المهام الدورية.\x1b[0m');
...
if (IS_LEADER) store.getPendingScheduledNotifications().forEach(armNotificationTimer);
...
if (!BACKUP_UPLOAD_URL && process.env.NODE_ENV === 'production') {
  ...
  // (إصلاح) في الإنتاج ده خطر فقدان بيانات كامل، مش تحذير. لازم قرار صريح.
  if (process.env.ALLOW_LOCAL_ONLY_BACKUP !== '1') {
    console.error('\x1b[31m⛔ التشغيل اتوقف: ظبّط BACKUP_UPLOAD_URL (نسخة احتياطية خارج السيرفر) أو شغّل ALLOW_LOCAL_ONLY_BACKUP=1 لو أنت متحمّل المخاطرة.\x1b[0m');
    process.exit(1);
  }
}
...
// ---------------------------------------------------------------------------
// (إصلاح) أصل الموقع: ترويسة Host مزوّرة كانت بتتسرّب لروابط sitemap/robots
// والميتا الاجتماعية. الأولوية دلوقتي لإعداد صريح، والـ Host بيتقبل بس لو
// موجود في ALLOWED_HOSTS (أو لو مفيش إعداد أصلًا — بيئة تطوير).
// ---------------------------------------------------------------------------
const CONFIGURED_BASE_URL = String(process.env.SITE_URL || process.env.PUBLIC_BASE_URL || '').trim().replace(/\/+$/, '');
const ALLOWED_HOSTS = new Set(
  String(process.env.ALLOWED_HOSTS || '')
    .split(',').map((h) => h.trim().toLowerCase()).filter(Boolean)
);
if (CONFIGURED_BASE_URL) {
  try { ALLOWED_HOSTS.add(new URL(CONFIGURED_BASE_URL).host.toLowerCase()); } catch (_) { /* تجاهل */ }
}
function siteBaseUrl(req) {
  if (CONFIGURED_BASE_URL) return CONFIGURED_BASE_URL;
  const host = String((req && req.get && req.get('host')) || '').toLowerCase();
  if (ALLOWED_HOSTS.size && !ALLOWED_HOSTS.has(host)) return '';
  return `${req.protocol}://${req.get('host')}`;
}
// (إصلاح) decodeURIComponent بيرمي URIError على مسار زي /%  → 500 غير مبرر.
...
const createRateLimiter = (options) => rateLimitFactory.createRateLimiter(
  MULTI_INSTANCE ? { ...options, persist: true } : options
);

// تنظيف دوري للسجلات المنتهية (حدود المعدّل + توكنات المصادقة).
setInterval(() => {
  rateLimitFactory.sweep();
  try { store.purgeExpiredRateLimits(); store.purgeExpiredAuthTokens(); store.purgeExpiredRevokedSessions(); } catch (_) { /* لا شيء */ }
}, 10 * 60 * 1000).unref();
...
// (إصلاح) عدّاد فشل الدخول لكل حساب: بيتخزّن في نفس مخزن الحدود الدائم،
// لكن بيتزوّد عند الفشل بس وبيتصفّر عند نجاح الدخول.
const ACCOUNT_FAILURE_WINDOW_MS = 15 * 60 * 1000;
const ACCOUNT_FAILURE_MAX = 10;
function accountFailureState(key) {
  try {
    const saved = store.rateLimitGet(key);
    if (saved && saved.resetAt > Date.now() && saved.count >= ACCOUNT_FAILURE_MAX) {
      return { locked: true, retryAfter: Math.max(1, Math.ceil((saved.resetAt - Date.now()) / 1000)) };
    }
  } catch (_) { /* الحد مش سبب لمنع الدخول */ }
  return { locked: false, retryAfter: 0 };
}
function registerAccountFailure(key) {
  try {
    const saved = store.rateLimitGet(key);
    const active = saved && saved.resetAt > Date.now();
    store.rateLimitSet(key, (active ? saved.count : 0) + 1, active ? saved.resetAt : Date.now() + ACCOUNT_FAILURE_WINDOW_MS);
  } catch (_) { /* تجاهل */ }
}
function clearAccountFailures(key) {
  try { store.resetRateLimit(key); } catch (_) { /* تجاهل */ }
}
...
// ---------------------------------------------------------------------------
// (تنظيف) الجلسات و CSRF بقوا في lib/session-layer.js
const sessionLayer = createSessionLayer({
  sessionKey: SESSION_KEY,
  csrfKey: CSRF_KEY,
  sessionCookie: SESSION_COOKIE,
  sessionMaxAgeMs: SESSION_MAX_AGE_MS,
  adminSessionMaxAgeMs: ADMIN_SESSION_MAX_AGE_MS,
});
const {
  CSRF_COOKIE, CSRF_HEADER,
  parseSessionToken, parseCookies,
  setSessionCookie, clearSessionCookie,
  isValidCsrfToken, issueCsrfCookie,
} = sessionLayer;
...
function sessionMiddleware(req, res, next) {
  ...
  // (إصلاح) الجلسة اللي اتعمل لها تسجيل خروج مرفوضة فورًا حتى لو التوكن لسه
  // موقّع وصالح زمنيًا (سرقة كوكي بعد الخروج).
  let revoked = false;
  if (data && data.jti) {
    try { revoked = store.isSessionRevoked(data.jti); } catch (_) { revoked = false; }
  }
  const user = (data && !revoked) ? store.findUserById(data.userId) : null;
  const versionMatches = user && data.sv === (user.session_version || 0);
  if (data && (!user || !versionMatches || revoked)) {
    req.session = null;
    req.user = null;
...
app.use(createSecurityHeaders({ googleAuth, sensitivePaths: SENSITIVE_PATHS }));
...
app.use((req, res, next) => {
  const normalized = safeDecodePath(req.path).toLowerCase().replace(/\/+/g, '/');
...
app.get('/robots.txt', (req, res) => {
  ...
  // من غير أصل موثوق ما بنكتبش رابط sitemap (منع تسميم الفهرسة بترويسة Host).
  res.type('text/plain').send(base ? `${rules}\nSitemap: ${base}/sitemap.xml\n` : rules);
});

app.get('/sitemap.xml', (_req, res) => {
  const base = siteBaseUrl(_req);
  if (!base) return res.status(404).type('text/plain').send('sitemap unavailable');
...
function injectNonce(html, nonce) {
  // (إصلاح XSS) الـ nonce بيتحقن في <script> و<style> الداخليين، فـ CSP تقدر
  // ترفض أي <style> أو <script> يحقنه مهاجم من غير ما نكسر صفحات الموقع.
  return html
    .replace(/<script(?![^>]*\bsrc=)([^>]*)>/gi, (match, attrs) => (
      /\bnonce=/i.test(attrs) ? match : `<script${attrs} nonce="${nonce}">`
    ))
    .replace(/<style([^>]*)>/gi, (match, attrs) => (
      /\bnonce=/i.test(attrs) ? match : `<style${attrs} nonce="${nonce}">`
    ));
}
...
app.post('/api/auth/register', authLimiter, authAccountLimiter, async (req, res) => {
  ...
  try {
    ...
    // ويستخدمه كأنه بتاعه.
    await store.createUserAsync({ ...value, role: 'customer', emailVerified: false });
...
app.post('/api/auth/login', authLimiter, async (req, res) => {
  ...
  // (إصلاح DoS) حد الحساب بقى على المحاولات *الفاشلة* بس: الدخول الصحيح
  // بيعدّي دايمًا ويصفّر العدّاد، فمحدش يقدر يقفل حساب حد تاني بمحاولات غلط.
  const failureKey = `auth-account|account:${email}`;
  const lock = accountFailureState(failureKey);
  if (lock.locked) {
    res.setHeader('Retry-After', lock.retryAfter);
    return res.status(429).json({ error: 'محاولات دخول فاشلة كثيرة على هذا الحساب، حاول بعد قليل.' });
  }
  const user = await store.verifyPasswordAsync(email, password);
  if (!user) {
    registerAccountFailure(failureKey);
    return res.status(401).json({ error: 'البريد الإلكتروني أو كلمة المرور غير صحيحة' });
  }
  clearAccountFailures(failureKey);
...
app.post('/api/auth/logout', (req, res) => {
  // (إصلاح) الخروج بيسجّل الجلسة في قائمة الإبطال لحد ما تنتهي مدتها.
  const session = req.session;
  if (session && session.jti) {
    const maxAge = session.role === 'admin' ? ADMIN_SESSION_MAX_AGE_MS : SESSION_MAX_AGE_MS;
    try { store.revokeSession(session.jti, session.userId, (session.iat || Date.now()) + maxAge); }
    catch (error) { console.error('[logout] فشل إبطال الجلسة:', error.message); }
  }
  clearSessionCookie(res);
  res.json({ ok: true });
});
...
app.post('/api/auth/change-password', requireAuth, authLimiter, async (req, res) => {
  ...
  if (!(await store.verifyPasswordAsync(req.user.email, currentPassword))) {
    return res.status(400).json({ error: 'كلمة المرور الحالية غير صحيحة' });
  }
  if (!newPassword || String(newPassword).length < 8) {
    return res.status(400).json({ error: 'كلمة المرور الجديدة يجب أن تكون 8 أحرف على الأقل' });
  }
  const user = await store.updateUserPasswordAsync(req.user.id, String(newPassword));
...
app.post('/api/auth/reset-password', passwordResetLimiter, async (req, res) => {
  ...
  await store.setUserPasswordAsync(user.id, String(password));
...
app.post('/api/auth/2fa/disable', requireAuth, authLimiter, async (req, res) => {
  const { password, code } = req.body || {};
  if (!(await store.verifyPasswordAsync(req.user.email, password))) {
...
app.post('/api/orders', requireAuth, writeLimiter, (req, res) => {
  ...
  if (PROOF_REQUIRED_METHODS.includes(value.paymentMethod)) {
    ...
    // (إصلاح احتيال) رقم العملية ما يتكررش في أكتر من طلب: نفس السكرين شوت
    // برقم عملية قديم بيتكشف فورًا بدل ما يعدّي على المراجعة اليدوية.
    if (ref && store.getOrderByTransferRef(ref)) {
      errors.push('رقم عملية التحويل ده مستخدم في طلب سابق. اكتب رقم العملية الجديدة الصح.');
    }
...
app.use((error, _req, res, _next) => {
  // (إصلاح) مسار مكسور زي /% بيرمي URIError جوه Express نفسه — ده طلب غلط من
  // العميل (400)، مش عطل في السيرفر (500)، ومش محتاج يتسجّل كـ error.
  const badRequest = error instanceof URIError || error.status === 400 || error.statusCode === 400
