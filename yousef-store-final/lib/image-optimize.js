...
// (إصلاح دفاع بالعمق) حد صريح لعدد البكسلات المسموح فك ضغطها — يمنع
// "decompression bomb" (صورة صغيرة الحجم بأبعاد ضخمة تستهلك كل الذاكرة).
...
async function optimizeInPlace(dir, filename) {
  ...
  try {
    await lib(source, { limitInputPixels: MAX_INPUT_PIXELS, failOn: 'error' })
      .rotate()
...
module.exports = { optimizeInPlace, isAvailable: () => !!loadSharp(), MAX_DIMENSION, QUALITY, MAX_INPUT_PIXELS };
