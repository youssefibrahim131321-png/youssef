 ...
 *  - قفل instance واحد: (إصلاح 7) مش بسبب SQLite تاني — القاعدة بقت Postgres
 *    مشتركة بطبيعتها. القفل ده بيحمي الملفات المحلية جوه DATA_DIR (كاش
 *    الصور/النسخ الاحتياطية/ملف كلمة السر الأولية) من تعارض كتابة بين
 *    نسختين على نفس المجلد. لو شغّلت instances فعلًا، فعّل ALLOW_MULTI_INSTANCE=1.
 */
const fs = require('fs');
...
function onCloudHost() {
  // (إصلاح) قابل للضبط: لو تخزينك دائم فعلًا (سيرفر خاص/Volume مثبّت) حطّ
  // PERSISTENT_STORAGE=1، من غير ما تضطر تعطّل الحماية كلها بـ
  // ALLOW_EPHEMERAL_STORAGE=1. الافتراضي لسه «سحابي» = الأكثر أمانًا.
  if (process.env.PERSISTENT_STORAGE === '1') return false;
  return true;
}
  ...
  try {
    if (fs.existsSync(lockPath)) {
      ...
      if (alive && pid !== process.pid) {
        logger.error(`\x1b[31m⛔ في نسخة تانية من السيرفر شغالة (PID ${pid}) على نفس مجلد البيانات المحلي. لو ده تعدد instances متعمّد على نفس الـ Volume، فعّل ALLOW_MULTI_INSTANCE=1 (قاعدة البيانات Postgres مشتركة أصلًا وبتستحمل تعدد النسخ).\x1b[0m`);
