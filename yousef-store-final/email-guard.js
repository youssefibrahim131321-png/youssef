...
// (إصلاح التعليق) استعلام DNS من غير مهلة ممكن يفضل مستني لدقايق على بعض
// السيرفرات، والنتيجة إن زرار "إنشاء حساب" يفضل "لحظات..." للأبد. أي استعلام
// بياخد أكتر من المهلة دي بنعتبره عطل شبكة (fail-open) ونكمّل التسجيل.
const MX_LOOKUP_TIMEOUT_MS = Number(process.env.EMAIL_GUARD_MX_TIMEOUT_MS || 3000);
...
async function hasMailExchanger(domain) {
  ...
  let ok = false;
  try {
    const records = await withTimeout(dns.resolveMx(domain), MX_LOOKUP_TIMEOUT_MS, 'resolveMx');
    ok = Array.isArray(records) && records.some((r) => r && r.exchange);
  } catch (err) {
    if (err && err.code === 'ETIMEOUT') throw err; // عطل شبكة → fail-open فوق
    // بعض النطاقات بتستقبل بريد على سجل A لو مفيش MX
    try {
      const a = await withTimeout(dns.resolve4(domain), MX_LOOKUP_TIMEOUT_MS, 'resolve4');
      ok = Array.isArray(a) && a.length > 0;
    } catch (_e) { if (_e && _e.code === 'ETIMEOUT') throw _e; ok = false; }
  }
...
async function checkEmail(email) {
  ...
  // (إصلاح) كتير من مزوّدي البريد المؤقت بيدّوا نطاقات فرعية عشوائية،
  // فبنفحص النطاق نفسه وكل نطاقاته الأب.
  const domainChain = domain.split('.').map((_, i, parts) => parts.slice(i).join('.'));
  if (domainChain.some(d => DISPOSABLE.has(d))) {
  ...
  if (process.env.EMAIL_GUARD_MX !== '0') {
    ...
    try {
      ...
      // محاولة تانية سريعة قبل ما نعتبرها عطل شبكة ونعدّي.
      try {
        await new Promise(r => setTimeout(r, 300));
        deliverable = await hasMailExchanger(domain);
        if (!deliverable) return { ok: false, reason: 'نطاق البريد ده مش بيستقبل رسائل — تأكد من كتابة البريد صح.' };
      } catch (_e) {
