...
function createStore(jsonDbPathHint) {
  ...
  function ensureAdmin({ email, password }) {
    return tx(() => {
      ...
      return { created: true, email: target, usingDefaultPassword: !password };
    });
  }
  ...
  function createUser({ name, email, password, role = 'customer', phone = '', address = '', emailVerified = false }) {
    ...
    const info = db.prepare(`INSERT INTO users (name, email, normalized_email, password_hash, role, phone, address, must_change_password, email_verified, created_at)
                              VALUES (?, ?, ?, ?, ?, ?, ?, 0, ?, ?)`)
      .run(String(name).trim().slice(0, 80), target, require('./email-guard').normalizeEmail(target), bcrypt.hashSync(password, 10), role, String(phone || '').slice(0, 30), String(address || '').slice(0, 300), emailVerified ? 1 : 0, nowISO());
  ...
  return {
    ...
    createAuthToken, consumeAuthToken, createAuthCode, consumeAuthCode, invalidateAuthTokens, purgeExpiredAuthTokens, markEmailVerified, setUserPassword,
    // التحقق بخطوتين
    ...
    // طلبات
    createOrder, getOrders, queryOrders, getOrdersForExport, getRecentOrders, getOrdersByUser, getOrderById, updateOrder, getOrderByProofFilename,
