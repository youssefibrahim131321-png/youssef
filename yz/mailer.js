...
async function sendMail({ to, subject, text, link }) {
  ...
  try {
    if (provider === 'resend') return await sendViaResend({ to, subject, text, link });
    if (provider === 'smtp') return await sendViaSmtp({ to, subject, text, link });
    if (provider === 'webhook') return await sendViaWebhook({ to, subject, text, link });
  } catch (error) {
  ...
  if (IS_PROD) {
    ...
    console.error('   ظبّط RESEND_API_KEY أو SMTP_URL أو MAIL_WEBHOOK_URL في متغيرات البيئة.\x1b[0m');
