...
(function initCanvas() {
  ...
  function drawFrame() {
    ...
    particles.forEach((p) => {
      ...
      ctx.fillStyle = `rgba(200, 121, 63, ${p.o})`;
      ctx.fill();
      ...
      bucket.forEach((p, i) => {
        ...
        candidates.forEach((p2) => {
          ...
          if (dist < LINK_DIST) {
            ...
            ctx.strokeStyle = `rgba(200, 121, 63, ${0.08 * (1 - dist / LINK_DIST)})`;
            ctx.stroke();
          }
        });
      });
...
$('burgerBtn')?.addEventListener('click', () => {
  ...
  nav.style.cssText = open ? '' : 'display:flex;position:fixed;top:var(--header-h);inset-inline:0;background:rgba(2,2,4,0.97);flex-direction:column;padding:24px;gap:20px;border-bottom:1px solid var(--line);z-index:199;backdrop-filter:blur(20px);';
